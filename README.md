...
1. 业务端初始化：注册回调函数void GomokuServer::initializeRouter()
把 lambda 以 {GET, "/"} 为 key 存入 callbacks_。
2. 请求到达，进行请求分割（context, req, resp）
   ssl处理
tls的RSA密钥交换（TLS1.2及以下）：服务器发公钥 → 客户端造随机数 → 公钥加密发送 → 服务器私钥解密得会话密钥 → 后续对称加密
公钥加密，私钥解密（一个证书只有一对公/私钥，私钥可能泄露，不具有前向安全性）：基于大整数分解的不可逆性
tls的DH密钥交换（主流版本）：服务器和客户端各自生成私钥 → 交换幂运算结果 → 幂运算计算共享秘钥 → 后续对称加密
DH加密：基于大素数幂运算可解性和离散对数不可解性（不可逆性）
   中间件：CORS预检：浏览器跨域检查请求
3. 路由（url->handler）
业务端处理请求：在 HttpServer 中调用 Router::route 函数，根据请求方法和路径匹配回调函数，执行具体业务逻辑，包括session管理（创建、获取、更新、销毁）、封装响应体。
4. 封装响应头，CORS处理、ssl加密等
...
1. 中间件
鉴权认证    检查 Token/Session，未登录直接返回 401
访问日志	   记录请求方法、路径、来源 IP	记录响应状态码、耗时
限流        检查IP/用户请求频率，超限返回 429
请求体解析	 把 JSON body 反序列化为对象放入上下文
压缩 Gzip	压缩响应 body，设置 Content-Encoding: gzip
静态缓存	   检查文件是否有缓存命中，命中则返回 304	设置 Cache-Control、ETag等
CSRF 防护	校验 Token，不匹配返回 403
XSS 防护	   设置 X-Content-Type-Options: nosniff 等安全头
请求超时	   设置超时定时器
参数校验	   检验请求参数格式
...
使用连接池，只会在程序启动连接池初始化的时候将执行连接建立的操作，在程序退出的时候执行连接销毁的操作。在程序运行的过程中需要执行数据库操作时，可以直接从连接池中取出现成的连接使用，使用完毕(执行完sql语句)后再将连接放回连接池即可。