...
// 这是一个 HTTP 协议解析状态机，核心作用是将 TCP 接收到的原始字节流逐步解析为结构化的 HttpRequest 对象。
// state:  kExpectRequestLine  →  kExpectHeaders  →  kExpectBody  →  kGotAll
bool HttpContext::parseRequest(Buffer *buf, Timestamp receiveTime)
{
    ...
    while (hasMore)
    {
        ...
        else if (state_ == kExpectHeaders)
        {
            ...
            if (crlf)
            {
                // 查找':' eg: Host: example.com
                const char *colon = std::find(buf->peek(), crlf, ':');
                if (colon < crlf) 
                {
                    // 找到':'，说明是Header行
                    // 以 : 为界，左边的子串作为 key，右边的子串（去掉前后空格）作为 value，存入 map<string, string> headers_。
